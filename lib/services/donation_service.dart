    ...
    try {
      final paymentData = {
        'transaction_details': {
          ...
          'gross_amount': donation.amount.toInt(),
        },
        'customer_details': {
          'first_name': userName,
          'email': userEmail,
        ...
        'item_details': [
          {
            'id': community.id,
            'price': donation.amount.toInt(),
            'quantity': 1,
            'name': 'Donasi Carbon Offset - ${community.name}',
            'category': 'donation',
          }
        ],
        'credit_card': {
        ...
        'callbacks': {
          'finish': 'ecotrack://donation/success',
        },
      };

      final response = await http.post(
        Uri.parse(_midtransBaseUrl),
        headers: {
          'Accept': 'application/json',
          'Content-Type': 'application/json',
          'Authorization': 'Basic ${base64Encode(utf8.encode('$_midtransServerKey:'))}',
        },
      ...
      if (response.statusCode == 201) {
        ...
        return responseData['redirect_url'] as String;
      } else {
        final errorData = json.decode(response.body);
        throw app_auth.AppAuthException(
          'Gagal membuat pembayaran: ${errorData['error_messages']?.join(', ') ?? 'Unknown error'}',
  ...
  Future<Map<String, dynamic>> checkPaymentStatus(String orderId) async {
    try {
      ...
      if (response.statusCode == 200) {
        ...
        throw app_auth.AppAuthException('Gagal memeriksa status pembayaran', 'payment_status_check_failed');
      }
    } catch (e) {
      if (e is app_auth.AppAuthException) rethrow;
